                    ...
                    <div class="card-body">
                        @if ($errors->any())
                            <div class="alert alert-danger mb-4 rounded">
                                <h5 class="font-weight-bold"><i class="fas fa-exclamation-triangle mr-2"></i> Gagal Menyimpan!
                                </h5>
                                <ul class="mb-0">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="description" id="description" class="form-control"
                                placeholder="Beli apa? / Dari siapa?" required>
                        </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="input-group">
                                        ...
                                        <input type="text" id="amount_display"
                                            class="form-control form-control-lg numeric-input" required>
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="from_account_id" id="from_account_id" class="form-control select2">
                                        ...
                                        @foreach ($accounts as $acc)
                                            <option value="{{ $acc->id }}" data-category="{{ $acc->category }}"
                                                data-type="{{ $acc->type }}">{{ $acc->code }} - {{ $acc->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                </div>
                        ...
                        <div id="investment_fields" class="mt-3 p-3 bg-light border rounded dynamic-field-section"
                            style="display:none;">
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-3">
                                    <div class="form-group">
                                        ...
                                        <input type="number" name="quantity" class="form-control" step="0.000001"
                                            placeholder="0.00">
                                    </div>
                                </div>
                            </div>
                        ...
                        <div id="debt_fields" class="mt-3 p-3 bg-light border rounded dynamic-field-section"
                            style="display:none;">
                            <h6 class="font-weight-bold mb-3 text-danger"><i class="fas fa-credit-card mr-1"></i> Detail
                                Pembayaran Hutang / Tagihan</h6>
                            <div class="row">
                                <div class="col-md-12 mb-3">
                                    <div class="alert alert-info py-2 small">
                                        <i class="fas fa-info-circle mr-1"></i> Fitur ini akan mencatat <strong>Pengurangan
                                            Hutang</strong> (limit kembali) sekaligus mencatatnya sebagai
                                        <strong>Pengeluaran</strong> bulan berjalan.
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="form-group">
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <select name="link_type" id="link_type" class="form-control mb-2"
                                            onchange="toggleLinkageFields()">
                                        ...
                                        <div id="link_installment_container" style="display:none;">
                                            <select name="installment_id" class="form-control select2">
                                                ...
                                                @foreach ($activeInstallments as $inst)
                                                    <option value="{{ $inst->id }}">{{ $inst->name }} (Sisa
                                                        {{ $inst->remaining_periods }} bln)</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        ...
                                        <div id="link_recurring_container" style="display:none;">
                                            <select name="recurring_transaction_id" class="form-control select2">
                                                ...
                                                @foreach ($recurringTransactions as $rt)
                                                    <option value="{{ $rt->id }}">{{ $rt->name }} (Rp
                                                        {{ number_format($rt->amount, 0, ',', '.') }})</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
    ...
    <script>
        ...
        function filterAccountOptions(selectId, categories) {
            ...
            select.find('option').each(function() {
                ...
                let cat = opt.data('category');
                if (categories.includes(cat)) {
                    opt.prop('disabled', false);
                } else {
                    opt.prop('disabled', true);
                    if (opt.val() === currentValue) select.val("");
                }
            });
        ...
        function toggleFormFields() {
            ...
            if (type === 'expense') {
                ...
                debtFields.show();
                filterAccountOptions('#from_account_id', ['asset', 'liability']);
                filterAccountOptions('#to_account_id', ['expense']);
            } else if (type === 'income') {
                labelFrom.text('Sumber Pemasukan (Dari siapa?)');
                labelTo.text('Tujuan Dana (Masuk ke mana?)');
                filterAccountOptions('#from_account_id', ['income']);
                filterAccountOptions('#to_account_id', ['asset']);
            } else if (type === 'initial_balance') {
                labelFrom.text('Sumber Modal / Equity (Kredit)');
                labelTo.text('Tujuan Harta / Saldo (Debit)');
                filterAccountOptions('#from_account_id', ['equity']);
                filterAccountOptions('#to_account_id', ['asset']);
            } else if (type === 'transfer') {
                labelFrom.text('Rekening Sumber');
                labelTo.text('Rekening Tujuan');
                filterAccountOptions('#from_account_id', ['asset', 'liability']);
                filterAccountOptions('#to_account_id', ['asset', 'liability']);
                ...
                debtFields.show();
                filterAccountOptions('#from_account_id', ['asset']);
                filterAccountOptions('#to_account_id', ['liability']);
            }
        }
